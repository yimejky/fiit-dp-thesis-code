...
class UNet(nn.Module):
    ...
    def __init__(self, in_channels=8):
        ...
        # downconv1
        self.dconv_down1 = double_conv(1, out_channels, padding=(1, 1, 1))
        self.pool1 = nn.MaxPool3d(2, stride=2, ceil_mode=True)  # 1/2

        # downconv2
        self.dconv_down2 = double_conv(out_channels, out_channels * 2)
        out_channels *= 2
        self.pool2 = nn.MaxPool3d(2, stride=2, ceil_mode=True)  # 1/4

        # downconv3
        self.dconv_down3 = double_conv(out_channels, out_channels * 2)
        out_channels *= 2
        self.pool3 = nn.MaxPool3d(2, stride=2, ceil_mode=True)  # 1/8

        # downconv4
        self.dconv_down4 = double_conv(out_channels, out_channels * 2)
        out_channels *= 2
        self.pool4 = nn.MaxPool3d(2, stride=2, ceil_mode=True)  # 1/16

        # middle
        self.dconv_middle = double_conv(out_channels, out_channels * 2)
        out_channels *= 2
        print(f'max output channels {out_channels}')
        
        # upconv1
        self.up1 = nn.ConvTranspose3d(out_channels, out_channels, 2, stride=2, bias=False)
        self.dconv_up1 = double_conv(out_channels + out_channels // 2, out_channels // 2)
        out_channels = out_channels // 2

        # upconv2
        self.up2 = nn.ConvTranspose3d(out_channels, out_channels, 2, stride=2, bias=False)
        self.dconv_up2 = double_conv(out_channels + out_channels // 2, out_channels // 2)
        out_channels = out_channels // 2
        
        # upconv3
        self.up3 = nn.ConvTranspose3d(out_channels, out_channels, 2, stride=2, bias=False)
        self.dconv_up3 = double_conv(out_channels + out_channels // 2, out_channels // 2)
        out_channels = out_channels // 2

        # upconv4
        self.up4 = nn.ConvTranspose3d(out_channels, out_channels, 2, stride=2, bias=False)
        self.dconv_up4 = double_conv(out_channels + out_channels // 2, out_channels // 2)
        out_channels = out_channels // 2
